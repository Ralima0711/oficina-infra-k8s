...
Provisionar, de forma reprodutível, o cluster EKS que executa a aplicação e o **Kong** (API Gateway, rodando no próprio cluster) que roteia e protege as rotas. Isolado do banco (repo `oficina-infra-database`) e da aplicação para permitir CI/CD independente.
...
| Kong (no EKS) | API Gateway open-source: roteamento e proteção das rotas |
...
| Kong | Rota pública `/auth`; demais rotas exigem Bearer token |
...
```
Kong (API Gateway) ──▶ EKS (pods da API + HPA) ──▶ RDS (repo oficina-infra-database)
```
